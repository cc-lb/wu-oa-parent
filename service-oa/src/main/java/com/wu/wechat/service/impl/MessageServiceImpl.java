...
public class MessageServiceImpl implements MessageService {
    ...
    public void pushPendingMessage(Long processId, Long userId, String taskId) {
      //  Process process = processService.getById( processId);
        ...
        if(StringUtils.isEmpty(openid)) {
            openid = "oToE36pBrZSyKukTr9iY9sgQsec0";
        }
        WxMpTemplateMessage templateMessage = WxMpTemplateMessage.builder()
                .toUser(openid)//要推送的用户openid
                .templateId("KvOVeW7jz4-DZgQ_WuXjMZO5I4pPA7L7fflVNwC_ZQg")//模板id---》上传给微信公众号的模板id
    ...
    public void pushProcessedMessage(Long processId, Long userId, Integer status) {
        ...
        ProcessTemplate processTemplate = processTemplateService.getById(process.getProcessTemplateId());
        SysUser sysUser = sysUserService.getById(userId);
        SysUser currentSysUser = sysUserService.getById(LoginInfoHelper.getUserId());
        String openid = sysUser.getOpenId();
        if(StringUtils.isEmpty(openid)) {
